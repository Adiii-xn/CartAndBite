...
public class AdminRiderController {
    ...
    @FXML
    protected void addRider(ActionEvent event){
        ...
        scaleTransition.setToX(1.1);
        scaleTransition.setToY(1.1);
        scaleTransition.setOnFinished(e -> overlayAddRider(event, "/com/system/AdminPack/AdminRider/AddRiderFX.fxml"));
        scaleTransition.play();

    }


    // Overlay for Add Rider
    private void overlayAddRider(ActionEvent event, String fxmlPath) {
        try {
            ...
            // Get the controller of the popup (LogoutController)
            AddRiderController addRiderController = loader.getController();
            addRiderController.setParentStage((Stage) addRiderBtn.getScene().getWindow());  // Pass the AdminDashboard stage
